...
const perm make_permid() {
    perm res;
    for (uint64_t i=0; i<Size; i++) set(res, i) = i;
    return res;
}
...
std::vector<perm> rand_perms(int sz) {
    ...
    for (int i = 0; i < sz; i++) res[i] = random_perm();
    return res;
}
...
perm permute_3(const perm &v1, const perm &v2) {
	...
	for (uint64_t i = 1; i < NBlock; i++) {
	  v2j -= 16;
	  res[j] = _mm_blendv_epi8(res[j], _mm_shuffle_epi8(v1[i], v2j), v2j <= 15);
	}
    }
    return res;
}
...
perm permute_ref(const perm &v1, const perm &v2) {
    perm res;
    for (uint64_t i=0; i < Size; i++) set(res, i) = get(v1, get(v2, i));
    return res;
}
...
int main() {
    ...
    */
    ...
    timethat([&vrand, &check_1]() {
	    transform(vrand.begin(), vrand.end(), check_1.begin(),
			   [](perm p) {
                               for (int i=0; i < repl; i++)
				   p = permute_1(p, p);
			       return p;
    ...
    timethat([&vrand, &check_2]() {
	    transform(vrand.begin(), vrand.end(), check_2.begin(),
			   [](perm p) {
                               for (int i=0; i < repl; i++)
				   p = permute_2(p, p);
			       return p;
    ...
    timethat([&vrand, &check_3]() {
	    transform(vrand.begin(), vrand.end(), check_3.begin(),
			   [](perm p) {
                               for (int i=0; i < repl; i++)
				   p = permute_3(p, p);
			       return p;
    ...
    assert(mismatch(check_ref.begin(), check_ref.end(),
			 check_1.begin(), eqperm) ==
	   make_pair(check_ref.end(), check_1.end()));
    assert(mismatch(check_ref.begin(), check_ref.end(),
    			 check_2.begin(), eqperm) ==
	   make_pair(check_ref.end(), check_2.end()));
    assert(mismatch(check_ref.begin(), check_ref.end(),
    			 check_3.begin(), eqperm) ==
	   make_pair(check_ref.end(), check_3.end()));
    cout << "Ok !" << endl;
}
